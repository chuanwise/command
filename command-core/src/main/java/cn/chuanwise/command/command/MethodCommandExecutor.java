...
public class MethodCommandExecutor
     ...
     * 指令
     */
    protected final cn.chuanwise.command.command.Command command;
    ...
    public boolean execute(CommandContext context) throws Exception {
        ...
        final CommandPreExecuteEvent commandHandlePreEvent = new CommandPreExecuteEvent(context);
        commander.getEventService().broadcastEvent(commandHandlePreEvent);
        ...
        for (int i = 0; i < wires.length; i++) {
            ...
            if (Objects.isNull(container) || container.isEmpty()) {
                commander.getEventService().broadcastEvent(new WireFailedEvent(context, i, parameter, wirer));
                return false;
            }
            ...
            if (Objects.nonNull(argument)) {
                if (!parameter.getType().isInstance(argument)) {
                    commander.getEventService().broadcastEvent(
        ...
        try {
            ...
            commander.getEventService().broadcastEvent(commandExecuteErrorEvent);
            return false;
        }

        final CommandPostExecuteEvent commandHandlePostEvent = new CommandPostExecuteEvent(context);
        commander.getEventService().broadcastEvent(commandHandlePostEvent);
        return true;
    }
