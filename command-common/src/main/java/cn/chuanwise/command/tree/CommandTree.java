...
public class CommandTree
    ...
    public boolean execute(DispatchContext dispatchContext) throws Exception {
        ...
        if (Objects.isNull(command) || Objects.isNull(command.getExecutor())) {
            commander.broadcastEvent(new UnhandledCommandEvent(dispatchContext, commandTreeNode));
            return false;
        }
        ...
        if (i != formatInfoElements.length) {
            ...
            for (int j = i; j < elements.size(); j++) {
                ...
                if (optionInfoKey.isSet()) {
                    commander.broadcastEvent(new ReassignOptionEvent(dispatchContext, optionInfo, optionInfoKey.getString(), string));
                    return false;
                }
            ...
            for (OptionInfoKey optionInfoKey : optionInfoKeys.values()) {
                ...
                if (Objects.isNull(string)) {
                    ...
                    if (Objects.isNull(defaultValue)) {
                        commander.broadcastEvent(new LackRequiredOptionEvent(dispatchContext, command, optionInfo));
                        return false;
                    }
                    ...
                    if (cn.chuanwise.common.util.Collections.nonEmpty(optionalValues)) {
                        if (!optionalValues.contains(string) && !configuration.isAllowUndefinedOptionValue()) {
                            commander.broadcastEvent(new UndefinedOptionValueEvent(dispatchContext, optionInfo, string, command));
                            return false;
                        }
                    }
    ...
    public CommandTreeFork dispatchSerially(DispatchContext dispatchContext) throws Exception {
        ...
        if (forks.isEmpty()) {
            ...
            commander.broadcastEvent(event);
            return null;
        }
        ...
        if (copiedForks.size() != 1) {
            ...
            if (Objects.isNull(singleStrongFork)) {
                commander.broadcastEvent(new MultipleCommandsMatchedEvent(dispatchContext, forks));
                return null;
            } else {
    ...
    public List<CommandTreeFork> dispatch(DispatchContext dispatchContext) throws Exception {
        ...
        for (int i = 1; i < arguments.size(); i++) {
            ...
            if (forks.isEmpty()) {
                commander.broadcastEvent(new MismatchedFormatEvent(dispatchContext, clonedForks));
                return Collections.emptyList();
            }
...
//                    final FormatInfo.Element.Reference.Option option = (FormatInfo.Element.Reference.Option) element;
    ...
    public void registerCommand(Command command) {
        ...
        commander.broadcastEvent(commandRegisterEvent);
